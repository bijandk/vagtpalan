...
function Login({onLogin,users}){
 ...
 function go(e){e.preventDefault();let u=users.find(x=>x.email.toLowerCase()===email.toLowerCase()&&x.password===pw);if(!u)return setErr("Forkert login");save("user-v14-2",u);onLogin(u)}
 return <div className="login"><div className="card"><div className="logo">🍕</div><h1>Surdejspizzeria</h1><p>V14.2 demo-login</p><form onSubmit={go}><label>Email</label><input value={email} onChange={e=>setEmail(e.target.value)}/><label>Kode</label><input type="password" value={pw} onChange={e=>setPw(e.target.value)}/>{err&&<div className="err">{err}</div>}<button className="dark"><Lock size={18}/> Log ind</button></form><div className="demo">{users.map(u=><button key={u.email} onClick={()=>{setEmail(u.email);setPw(u.password||"demo123")}}>{u.role==="admin"?<ShieldCheck size={15}/>:<User size={15}/>} {u.email}</button>)}</div></div></div>
}

function App(){
 const[employees,setEmployees]=useState(load("employees-v14-2",baseEmployees));
 const[users,setUsers]=useState(load("users-v14-2",baseUsers));
 const[user,setUser]=useState(load("user-v14-2",null));
 const[restaurant,setRestaurant]=useState(load("restaurant-v14-2","city"));
 const[plans,setPlans]=useState(load("plans-v14-2",defaultPlans));
 ...
 function logout(){localStorage.removeItem("user-v14-2");setUser(null)}
 ...
 function addShift(emp,d){if(!isAdmin&&emp!==user.name)return alert("Du kan kun tilføje egne vagter");let n=JSON.parse(JSON.stringify(plans));if(!n[restaurant])n[restaurant]={};if(!n[restaurant][emp])n[restaurant][emp]={};if(!n[restaurant][emp][d]||n[restaurant][emp][d][0]==="FRI")n[restaurant][emp][d]=["16:00–21:00\nNy vagt"];else n[restaurant][emp][d].push("16:00–21:00\nNy vagt");setPlans(n);save("plans-v14-2",n);let idx=n[restaurant][emp][d].length-1;open(emp,d,idx,n[restaurant][emp][d][idx])}
 function saveEdit(){let n=JSON.parse(JSON.stringify(plans));n[restaurant][edit.emp][edit.d][edit.i]=text.trim()||"FRI";setPlans(n);save("plans-v14-2",n);setEdit(null);setStatus("Gemt lokalt")}
 function del(){let n=JSON.parse(JSON.stringify(plans));n[restaurant][edit.emp][edit.d].splice(edit.i,1);if(!n[restaurant][edit.emp][edit.d].length)n[restaurant][edit.emp][edit.d]=["FRI"];setPlans(n);save("plans-v14-2",n);setEdit(null);setStatus("Slettet lokalt")}
 function createEmployee(e){
  ...
  save("employees-v14-2",nextEmp);save("users-v14-2",nextUsers);save("plans-v14-2",nextPlans);
  setNewEmp({name:"",email:"",role:"Pizzaman",weekly:37});setAddEmp(false);setStatus("Medarbejder tilføjet");
 }
 async function saveOnline(){if(!isAdmin)return alert("Kun admin");setStatus("Gemmer...");let data={plans,employees,users};let{error}=await supabase.from("schedules").upsert({id:"surdejspizzeria-v14-2",data,updated_at:new Date().toISOString()});setStatus(error?"Fejl: "+error.message:"Gemt online")}
 async function loadOnline(){setStatus("Henter...");let{data,error}=await supabase.from("schedules").select("data").eq("id","surdejspizzeria-v14-2").maybeSingle();if(error)return setStatus("Fejl: "+error.message);if(!data)return setStatus("Ingen online data");setPlans(data.data.plans||defaultPlans);setEmployees(data.data.employees||baseEmployees);setUsers(data.data.users||baseUsers);save("plans-v14-2",data.data.plans||defaultPlans);save("employees-v14-2",data.data.employees||baseEmployees);save("users-v14-2",data.data.users||baseUsers);setStatus("Hentet online")}

 return <div className="app">
  <header><div><b>🍕 Surdejspizzeria</b><h1>Vagtplan {info.name}</h1><p>{info.address} · V14.2 tilføj medarbejder · {user.email}</p></div><div className="sync"><span><Database size={16}/> {status}</span><button onClick={loadOnline}><DownloadCloud size={16}/> Hent</button>{isAdmin&&<button className="dark" onClick={saveOnline}><UploadCloud size={16}/> Gem online</button>}<button onClick={logout}><LogOut size={16}/> Log ud</button></div></header>
  <section className="switch">{restaurants.map(r=><button className={restaurant===r.id?"active":""} key={r.id} onClick={()=>{setRestaurant(r.id);save("restaurant-v14-2",r.id)}}><Store size={18}/> {r.name}</button>)}</section>
  ...
  <div className="note"><UserPlus size={19}/> V14.2: Admin kan tilføje medarbejder direkte i appen. Ny medarbejder får demo-kode: demo123.</div>
